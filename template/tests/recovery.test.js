...
describe('Recovery Flow', () => {
  ...
  describe('POST /api/recovery/verifyCode', () => {
    ...
    it('should log a security warning if method is invalid', async () => {
      const logger = (await import('../src/infrastructure/logger.js')).default;
      const warnSpy = jest.spyOn(logger, 'warn');

      const res = await request(app)
        .post('/api/recovery/recoveryCode')
        .send({ method: 'invalid', email });

      expect(res.status).toBe(400);
      expect(warnSpy).toHaveBeenCalledWith(
        'Intento de recuperación con método inválido',
        { method: 'invalid' }
      );
      warnSpy.mockRestore();
    });
  ...
  describe('POST /api/recovery/verifyCode', () => {
    it('should log a security warning on invalid code', async () => {
      const logger = (await import('../src/infrastructure/logger.js')).default;
      const warnSpy = jest.spyOn(logger, 'warn');
      ...
      expect(warnSpy).toHaveBeenCalledWith(
        'Intento fallido de verificación de código',
