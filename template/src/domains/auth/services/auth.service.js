...
export class AuthService {
  ...
  async registerUser({ email, password, phone, verificationMethod = 'email' }) {
    ...
    logger.info('Usuario registrado exitosamente', { userId: newUser.id, email });
    return {
      id: newUser.id,
      email: newUser.email,
  ...
  async loginUser({ email, password }) {
    ...
    if (user.lockUntil && user.lockUntil > Date.now()) {
      logger.warn('Intento de login en cuenta bloqueada', { userId: user.id, email });
    ...
    if (!user.isVerified) {
      logger.warn('Intento de login en cuenta no verificada', { userId: user.id, email });
    ...
    if (!isMatch) {
      logger.warn('Intento fallido de inicio de sesión', { userId: user.id, email });
      user.loginAttempts += 1;
      if (user.loginAttempts >= 5) {
        user.lockUntil = new Date(Date.now() + 30 * 60 * 1000);
        logger.warn('Cuenta bloqueada', { userId: user.id, email });
      }
      await this.userRepository.save(user);
    ...
    logger.info('Usuario inició sesión exitosamente', { userId: user.id, email });
  ...
  async verifyMFA(mfaToken, code) {
    try {
      ...
      logger.info('MFA verificado exitosamente', { userId: user.id });
      return await this._generateAndStoreTokens(user);
    } catch (err) {
      logger.warn('Intento fallido de verificación MFA', { mfaToken, error: err.message });
  ...
  async verifyAccount({ identifier, code, method }) {
    ...
    if (!user) {
      logger.warn('Intento de verificación de cuenta con usuario inexistente', { identifier });
    ...
    if (!user.verificationCode || user.verificationCodeExpires < Date.now()) {
      logger.warn('Intento de verificación con código expirado', { userId: user.id });
    ...
    if (!isMatch) {
      logger.warn('Intento de verificación con código incorrecto', { userId: user.id });
  ...
  async resendVerificationCode({ identifier, method }) {
    ...
    if (!user) {
      logger.warn('Intento de reenvío de código con usuario inexistente', { identifier });
      const error = new Error('Usuario no encontrado');
      error.statusCode = 404;
      throw error;
    }
    ...
    logger.info('Código de verificación reenviado', { userId: user.id, method });
    return { message: `Nuevo código enviado vía ${method}` };
  }

  async refreshAccessToken(token) {
    if (!token) {
      logger.warn('Intento de refresco de token sin token');
    ...
    try {
      ...
      if (!user || tokenIndex === -1) {
        logger.warn('Intento de refresco de token con token inválido', { userId: decoded.id });
      ...
      logger.info('Token de acceso refrescado exitosamente', { userId: user.id });
      return { accessToken: newToken, refreshToken: newRefreshToken };
    } catch (err) {
      logger.warn('Error al refrescar token', { error: err.message });
  ...
  async logoutUser(token) {
    if (token) {
      ...
      if (user) {
        ...
        logger.warn('Intento de logout con token inexistente');
      }
    }
  }
  ...
  async googleAuthSuccess(user) {
    if (!user) {
      logger.error('Fallo en autenticación de Google: Usuario no encontrado');
      const error = new Error('Usuario no encontrado después de autenticación de Google');
      error.statusCode = 404;
      throw error;
    }
    logger.info('Usuario autenticado vía Google', { userId: user.id });
    return await this._generateAndStoreTokens(user);
  }
  ...
  async confirmMFASetup(userId, code) {
    ...
    if (user.twoFAType === 'app') {
      ...
      if (!verified) {
        logger.warn('Intento fallido de confirmación MFA', { userId });
        const error = new Error('Código de verificación incorrecto');
        error.statusCode = 400;
        throw error;
      }
    ...
    logger.info('MFA habilitado correctamente', { userId });
  ...
  async disableMFA(userId, password) {
    ...
    const isMatch = await this.passwordHasher.compare(password, user.password);
    if (!isMatch) {
      logger.warn('Intento fallido de deshabilitar MFA', { userId });
    ...
    logger.info('MFA deshabilitado correctamente', { userId });
